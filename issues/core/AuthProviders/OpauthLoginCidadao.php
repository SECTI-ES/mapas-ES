...
//            $_POST['openid_url'] = $config['login_url'];
    ...
    protected function _validateResponse(){
        ...
        if (array_key_exists('error', $response)) {
            ...
            $app->halt(500, \MapasCulturais\i::__('Opauth returns error auth response'));
        } else {
            /**
            ...
            */
            if (empty($response['auth']) || empty($response['timestamp']) || empty($response['signature']) || empty($response['auth']['provider']) || empty($response['auth']['uid'])) {
                $app->halt(500, \MapasCulturais\i::__('Invalid auth response: Missing key auth response components.'));
            } elseif (!$this->opauth->validate(sha1(print_r($response['auth'], true)), $response['timestamp'], $response['signature'], $reason)) {
                $app->halt(500, \MapasCulturais\i::__("Invalid auth response: {$reason}"));
            } else {
                $valid = true;
            }
        }
