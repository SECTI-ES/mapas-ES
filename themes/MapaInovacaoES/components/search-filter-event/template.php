...
<search-filter :position="position" :pseudo-query="pseudoQuery">
    ...
    <form class="form" @submit="$event.preventDefault()">
        ...
        <div class="field">
            ...
            <mc-multiselect :model="pseudoQuery['event:term:linguagem']" :items="terms" title="<?php i::_e('Selecione os tipos') ?>" hide-filter hide-button>
                <template #default="{popover, setFilter}">
                    <input class="mc-multiselect--input" @keyup="setFilter($event.target.value)" @focus="popover.open()" :triggers="['click']" placeholder="<?= i::esc_attr__('Selecione os tipos') ?>">
                </template>
            </mc-multiselect>
